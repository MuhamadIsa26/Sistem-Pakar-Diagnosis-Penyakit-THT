...
<head>
    ...
    <title>Pendaftaran Akun - Sistem Pakar THT</title>
    <!-- Tailwind CSS CDN -->
...
<body class="bg-slate-50 min-h-screen flex items-center justify-center p-4 sm:p-6 md:p-8">
    ...
    <!-- Card Container: Mobile-first dynamic spacing and sizes -->
    <div
        ...
        <div class="text-center mb-6">
            <div class="inline-flex p-3 bg-[#00685f]/10 text-[#00685f] rounded-full mb-3">
                <span class="material-symbols-outlined text-2xl sm:text-3xl font-bold">person_add</span>
            </div>
            <h2 class="text-xl sm:text-2xl font-bold text-[#171d1c] tracking-tight">Daftar Akun Baru</h2>
            <p class="text-xs sm:text-sm text-[#3d4947] mt-1.5 px-2">Buat akun untuk mulai melacak riwayat diagnosis
                kesehatan Anda.</p>
        </div>
        ...
        <form action="{{ route('register') }}" method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-slate-700 text-xs sm:text-sm font-semibold mb-1.5">Nama Lengkap Pasien</label>
                <div class="relative">
                    <span
                        class="material-symbols-outlined absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-base sm:text-lg">badge</span>
                    <!-- Min-height 44px for touch targets on mobile -->
                    <input type="text" name="name" value="{{ old('name') }}"
                        placeholder="Masukkan nama lengkap Anda..."
                        class="w-full pl-11 pr-4 py-2.5 sm:py-3 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#00685f]/50 text-xs sm:text-sm text-slate-800 transition-all min-h-[44px] @error('name') border-red-500 @enderror"
                        required autofocus>
                </div>
                @error('name')
            ...
            <div>
                <label class="block text-slate-700 text-xs sm:text-sm font-semibold mb-1.5">Username Akun</label>
                <div class="relative">
                    <span
            ...
            <div>
                <label class="block text-slate-700 text-xs sm:text-sm font-semibold mb-1.5">Konfirmasi Kata
                    Sandi</label>
                <div class="relative">
                    <span
                    ...
                    <input type="password" name="password_confirmation" placeholder="Ulangi kata sandi Anda..."
        ...
        <div class="text-center mt-6 pt-5 border-t border-slate-100">
            <p class="text-[11px] sm:text-xs text-[#3d4947] font-medium leading-relaxed">
                Sudah memiliki akun rekam medis?
                <a href="{{ route('login') }}"
